...
//#define CHESSBASE
#ifndef CHESSBASE
	#define HOUDINI_KEYMODIFIER
#endif
#ifdef HOUDINI_KEYMODIFIER
	#include <intrin.h>
#endif
...
int aes_setkey_enc( aes_context *ctx, const unsigned char *key, unsigned int keysize )
{
	...
	if ((unsigned int)CPUInfo[0] >= 0x80000002)
	{
		__cpuid(CPUInfo, 0x80000002);
		KeyModifier = CPUInfo[0] ^ CPUInfo[1] ^ CPUInfo[2] ^ CPUInfo[3];
	...
	{
		__cpuid(CPUInfo, 1);
		KeyModifier = CPUInfo[0] ^ CPUInfo[3];
	}
	RK = ctx->buf;
	for (i = 0; i < 64; i++)
		RK[i] ^= KeyModifier;
